...
func home(w http.ResponseWriter, r *http.Request) {
	...
	data, _ := connection.Conn.Query(context.Background(), "SELECT name, description, technologies, duration, id FROM tb_blog ORDER BY id ASC")
...
func register(w http.ResponseWriter, r *http.Request) {
	...
	_, err = connection.Conn.Exec(context.Background(), "INSERT INTO tb_user(name, email, password) VALUES ($1, $2, $3) WHERE email NOT IN (SELECT email FROM tb_user)", name, email, password)
	if err != nil {

		var store = sessions.NewCookieStore([]byte("SESSION_KEY"))
		session, _ := store.Get(r, "SESSION_KEY")

		session.AddFlash("Email sudah terdaftar!", "message")
		session.Save(r, w)
